...
# Donor fit
dat = fread("input/merged_crs_iati_keyword_search.csv")
...
fwrite(donor_fit, "output/donor_fit_kws.csv")
...
fwrite(dat_predict, "input/modeled_crs_iati_keyword_search.csv")

dat_predict_agg = dat_predict[,.(usd_disbursement_crs=sum(usd_disbursement_crs, na.rm=T)), by=.(year)]
library(ggplot2)
ggplot(dat_predict_agg, aes(x=year, y=usd_disbursement_crs)) +
